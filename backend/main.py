...
async def check_status(db: Session = Depends(get_db), user: models.users = Depends(get_current_user)):
    ...
    entry = db.query(models.game_player).filter(models.game_player.user_id == user.id).order_by(
        models.game_player.id.desc()).first()

    if entry:
        game = db.query(models.games).filter(models.games.id == entry.game_id, models.games.status == "playing").first()
        if game:
            return {"status": "playing", "game_id": game.id}
...
async def user_sync(db: Session = Depends(get_db), user: models.users = Depends(get_current_user)):
    ...
    # 1. Проверяем активную игру (для авто-перехода в поле)
    active_game = db.query(models.game_player).join(models.games).filter(
        models.game_player.user_id == user.id,
        models.games.status == "playing"
    ).first()

    # 2. Считаем количество активных инвайтов (для тостов)
    invites_count = db.query(models.invitations).filter(
        models.invitations.recipient_id == user.id,
        models.invitations.status == "pending"
    ).count()

    # 3. Считаем количество доступных лобби (для автообновления списка поиска)
...
async def read_fields(request: Request, db: Session = Depends(get_db), user: models.users = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("fields.html", ctx)